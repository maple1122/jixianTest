 ...
 */
public class MediaShare extends LoginPortal {
        ...
        try {
            ...
            for (int i = 0; i < 3; i++) {
                if (!CommonMethod.isJudgingElement(driver, By.tagName("header"))) {//校验是否跳转成功
                    driver.get("http://app.test.pdmiryun.com/mp/mpshare/mediaShare");//打开媒体号共享页面
                    Thread.sleep(2000);
                } else break;
            }
            ...
            if (!driver.findElement(By.xpath("//div[@class='nav-right']/ul/li/a")).getText().contains("爱富县")) {
                ...
                Thread.sleep(2000);
                driver.get("http://app.test.pdmiryun.com/mp/mpshare/mediaShare");//再次切换到媒体号共享
                Thread.sleep(2000);
            }
